...
class RubricOrchestrator:
    ...
    def orchestrate(self, study_profile: StudyProfile) -> List[RubricBlock]:
        """
        ...
        """
        # Step 1: Load all applicable rubrics
        rubric_items = self.rubric_loader.load_rubrics_for_study_types(
            study_profile.study_types
        )

        if not rubric_items:
            # Fallback: at least load universal rubric
